...
export function ApiSettings(props: { onSaved?: (config: ApiConfig | null) => void }) {
  const [config, setConfig] = useState<ApiConfig>(() => createDefaultApiConfig("openai"));
  ...
  function updateProvider(provider: Provider) {
    setConfig(createDefaultApiConfig(provider === "auto" ? "openai" : provider));
    setMessage("");
  }
  ...
  function remove() {
    clearApiConfig();
    const next = createDefaultApiConfig("openai");
    setConfig(next);
    props.onSaved?.(null);
    setMessage("已清除本机 API 配置。");
  }
    ...
    <div className="mt-3 border border-neutral-300 bg-neutral-50 p-3">
      ...
      <div className="grid gap-2">
        ...
        <label className="grid gap-1 text-sm">
          ...
          <input className="h-10 border border-neutral-300 bg-white px-3" value={config.baseUrl} onChange={(event) => setConfig({ ...config, baseUrl: event.target.value })} placeholder="https://api.openai.com/v1" />
        </label>
        <label className="grid gap-1 text-sm">
          <span className="font-bold">Model</span>
          <input className="h-10 border border-neutral-300 bg-white px-3" value={config.model} onChange={(event) => setConfig({ ...config, model: event.target.value })} placeholder="gpt-5.5" />
        </label>
      </div>
      ...
      <div className="mt-2 text-xs leading-5 text-neutral-500">
        API Key 只保存在当前浏览器 localStorage 中。本项目不提供统一 AI 配额，不上传数据库。
      </div>
    </div>
  );
}
